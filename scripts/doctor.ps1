...
if (Test-Path $venv) { & $venv -c "import fastapi,uvicorn,pandas,numpy,scipy,pydantic,pydantic_settings,multipart,httpx,pytest,webview,PyInstaller,openbb" 2>$null; Check 'Python packages' ($LASTEXITCODE -eq 0) $(if ($LASTEXITCODE -eq 0) { 'Installed, including OpenBB news runtime.' } else { 'Run .venv-local\\Scripts\\python.exe -m pip install -e .[dev] in backend.' }) }
