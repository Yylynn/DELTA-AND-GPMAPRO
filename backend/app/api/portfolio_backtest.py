...
def portfolio_backtest_export(request: PortfolioBacktestRequest):
    ...
    writer.writerow(["kind", "date", "symbol", "entry_date", "exit_date", "entry_price", "exit_price", "gross_return", "net_return", "equity", "cash", "open_positions", "snapshot_id", "data_sha256"])
    for row in result["equity_curve"]: writer.writerow(["equity", row["date"], "", "", "", "", "", "", "", row["equity"], row["cash"], row["open_positions"], provenance.get("snapshot_id", ""), provenance.get("data_sha256", "")])
    for trade in result["closed_trades"]: writer.writerow(["trade", "", trade["symbol"], trade["entry_date"], pd.Timestamp(trade["exit_date"]).date().isoformat(), trade["entry_price"], trade["exit_price"], trade["gross_return"], trade["net_return"], "", "", "", provenance.get("snapshot_id", ""), provenance.get("data_sha256", "")])
