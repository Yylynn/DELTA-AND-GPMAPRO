...
def futu_research_coverage():
    ...
    for item in futu_snapshots.list():
    ...
    return {"eligible_symbols": eligible, "required_symbols": len(RESEARCH_BASKET), "status": "READY" if eligible == len(RESEARCH_BASKET) else "NOT_READY", "symbols": rows}
...
def futu_snapshot_ohlcv(snapshot_id: str, timeframe: str = "1d"):
    try:
        _, manifest = futu_snapshots.load(snapshot_id)
        ...
        df, quality = bars_for_source(manifest["code"], timeframe, snapshot_id)
