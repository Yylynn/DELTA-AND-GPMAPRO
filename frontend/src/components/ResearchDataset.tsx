...
export function ResearchDataset() {
  ...
  const futuCoverage = useQuery<FutuCoverage>({ queryKey: ["market-research-coverage"], queryFn: () => request("/api/data/market/coverage") });
  ...
  const fetchEtfBasket = async () => {
    setFutuResult(null);
    setFutuResult(await request("/api/data/market/snapshots/batch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ codes: ["US.SPY", "US.QQQ", "US.IWM", "US.XLF", "US.XLE", "US.XLV", "US.XLI", "US.XLU"], timeframe: "1d", adjustment: "adjusted", start: "2010-01-01" }) }));
