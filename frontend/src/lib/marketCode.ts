...
export function resolveMarketCode(input: string, market: Market): string {
  ...
  if (value.includes(".")) {
    if (!QUALIFIED_CODE.test(value)) throw new MarketCodeError("请输入有效的市场代码，例如 SH.600519、SZ.000001、HK.00700 或 US.AAPL");
    return value;
  }
  if (market === "US") {
  ...
  if (market === "HK") {
    if (!/^\d{1,5}$/.test(value)) throw new MarketCodeError("港股代码应为 1 至 5 位数字，或输入完整市场代码");
    return `HK.${value.padStart(5, "0")}`;
  }
  if (!/^\d{6}$/.test(value)) throw new MarketCodeError("A 股代码应为 6 位数字，或输入完整市场代码");
  const prefix = value.slice(0, 3);
  if (SH_PREFIXES.has(prefix)) return `SH.${value}`;
  if (SZ_PREFIXES.has(prefix)) return `SZ.${value}`;
  throw new MarketCodeError("无法根据该 A 股代码判断沪深交易所，请输入完整市场代码（例如 SH.600519）");
}
